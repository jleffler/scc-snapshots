#!/bin/ksh
#
# @(#)$Id: scc.test-07.sh,v 8.1 2022/04/07 00:40:18 jleffler Exp $
#
...
do
    ...
    then : OK
        ...
        echo "Differences: $base-$standard.c - standard output (wanted vs actual)"
        diff "$EXPOUT" "$tmp.1"
        test=1
    fi
    if [ "$gflag" = yes ]
    ...
    then : OK
        ...
        echo "Differences: $base-$standard.c - standard error (wanted vs actual)"
        diff "$EXPERR" "$tmp.2"
        test=1
    fi
    if [ "$gflag" = yes ]
